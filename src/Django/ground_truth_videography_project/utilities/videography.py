...
def build_video(chunks, clip, audio_path, video_path):
    ...
    for i, chunk in enumerate(chunks):
        ...
        if text_clip:
            width = max(text_clip.w, image_clip.w)
            full_clip = CompositeVideoClip([create_buffer_clip((width, image_clip.h + 100), chunk.duration), image_clip, text_clip])
        else:
    ...
    video.write_videofile(video_path, fps=24, threads=8, codec='libx264')