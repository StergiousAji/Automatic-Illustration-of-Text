...
It was decided the project should be developed as a web application due to the system requirement \textbf{\#\ref{req:19}}. A web application can be easily deployed to the internet making it accessilbe to everyone regardless of the type of device used or operating system. Additionally, it has the benefit of not having the requirement of downloading large executable files.

Python \citep{python} was the main programming language chosen due to its flexibility and huge selection of libraries allowing rapid development of such a time-constrained project. Popular libraries such as NumPy \citep{harris2020numpy} offer a wide range of optimised functions for data processing and manipulation. Furthermore, this decision led to the idea of developing the web application using \cite{django}, a popular Python web framework.
...
\begin{itemize}
    \item The \textbf{Model} manages the the data stored in a relational database.
    \item The \textbf{View} communicates with both the model and template in order to complete and send back a HTTP response to the client corresponding to their HTTP request.
...
\subsubsection{Audio}
The Audio model will hold all information associated to inputted audio sources after processing is complete. This includes the artist/author name, title, transcript and the ground truth. A boolean flag for whether the audio is a music track or not will be used to course the appropriate processing steps. The file name field will be used to find corresponding files stored in the static directory, such as the audio, cover art image or the transcript files. 

\subsubsection{Chunk}
The Chunk model will consist of specific information about the segment in the audio such as the text present, start and end times and finally the top-10 and labelled images. It will hold a foreign key that references the appropriate Audio object.
