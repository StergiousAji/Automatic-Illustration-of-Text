...
(TODO: ADD PREVIOUS SYSTEM VIDEO)
\begin{figure}
    \centering
    % \includegraphics[width=1\textwidth]{figures/kyoto_quality_p.pdf}
    \includegraphics[width=1\textwidth]{figures/kyoto_quality_c.pdf}
    \caption{Comparison of the quality of videos generated by the previous (Left) and current (Right) videography systems.}
...
To exemplify, text prompts such as \emph{"Let it go"} or non-lexical words like \emph{"oh oh/la-la"} that are so common in song lyrics are particularly to delineate. It is also important to note that the ImageNet-1k dataset, while reasonably large consisting of over 1.1 million images, only covers 1 thousand \emph{'synsets'} as the name suggests. This means that it cannot cover niche concepts simply due to the fact that it does not have enough images that cover a broad enough ranage of ideas. This suggests that a larger image dataset could improve retrieval results, not to mention, utilising a larger pre-trained CLIP model.
