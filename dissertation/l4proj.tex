...
%==================================================================================================
\chapter{Design}
(TODO: REFERENCE REQUIREMENTS DIRECTLY AND CHECK CORRECT REFS)
...
\chapter{Conclusion}    
\section{Summary}
This project aimed to create two new technologies, namely a new Automated Videography system that improves on the previous product built by \cite{parker} and a ground-truth annotation interface. In the end, a successful system was produced that is capable of taking any audio source and generate a video comprising of relevant imagery to accompany the spoken language. Furthermore, a user-friendly interface was devised, which aimed to simplify the process of assessing the relevance of images derived from textual queries. This interface effectively streamlines this task of ground truth construction intended for the quantitative evaluation of automatic videography systems. To the best of our knowledge, this is something that is currently unavailable within existing solutions.
...
Firstly, the implementations of more efficient image indexing and retrieving methods were explored using FAISS which were unsuccessful. This did not cause a huge hindrance to the project since the system's exhaustive search was found to be sufficiently fast with the ImageNet-1k collection. Secondly, an comparison of the generation times of the current and preceding videography systems was done in order to evaluate our aim of decreasing the overal time taken. The results indicated that this system achieved a 42\% decrease in video generation time meaning that users do not have to wait as long. The final experiment involved usability testing of the ground-truth annotation interface with sixteen participants. The survey responses showed that on average, participants rated the intuitiveness of the interface, 4.4 out of 5, and every participant was able to complete their ground-truth building task. The completed ground-truths are made available in the project GitHub repository\footnote{\url{https://github.com/StergiousAji/Automatic-Illustration-of-Text}}. This data aims to enable smoother and repeatable performance evaluations of automatic videography systems and alleviate the need for subjective user evaluations.
