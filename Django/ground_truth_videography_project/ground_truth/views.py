...
from videography_pipeline.synced_lyrics_retriever import get_synced_lyrics
from videography_pipeline.image_retriever import CLIP, save_object, read_object
    ...
    if request.method == "POST":
        ...
        if audio_path:
            ...
            if title and artist:
                ...
                audio = Audio(artist=artist, title=title, filename=filename, transcript=transcript, coverart_colour=f"rgba{get_coverart_colour(filename, SRC_FOLDER)}")
            else:
                # TODO: HANDLE WHEN TITLE AND ARTIST NOT FOUND
                audio = Audio()
